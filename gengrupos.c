...
int main (int argc, char *argv[]) {
	...
	struct timespec  t11, t12, t17, t20, t21, t22;
	double tiempogc,tiemponc,tiempototal,tiempototal2;
	...
	while(ngrupos<MAX_GRUPOS && convergencia_cont<1){
		...
		while ((fin == 0) && (num_ite < MAXIT)) {
			...
			tiempogc=(end.tv_sec-start.tv_sec)+(end.tv_nsec-start.tv_nsec)/ 1.0e9;
			...
			clock_gettime(CLOCK_REALTIME,&end);
			tiemponc=(end.tv_sec-start.tv_sec)+(end.tv_nsec-start.tv_nsec)/ 1.0e9;						

			tiempototal+=tiempogc;
			tiempototal2+=tiemponc;
			num_ite++;
		}
		printf("Tiempo total de grupos cercanos %lf \n",tiempototal);
		printf("Tiempo total de nuevos centroides %lf \n",tiempototal2);
